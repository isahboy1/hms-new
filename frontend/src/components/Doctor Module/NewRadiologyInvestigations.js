...
export class NewRadiologyInvestigations extends Component {
    ...
    handleSubmit=(e)=> {
        ...
        return localStorage.setItem("lab_investigation", JSON.stringify(this.state.formRecords));
    }
    ...
    render(){
        ...
        return(
            <Form onSubmit={this.handleAddRequest}>
                <h3 className="text-center"><strong>Lab Investigation Request</strong></h3>
                <hr />
                <FormGroup row>
                    <label className="col-md-2">Required Investigation (and Others):</label>
                    <textarea name="test" value={this.state.test} onChange={this.onInputChange} className="form-control col-md-4" ></textarea>
                    <label className="col-md-2">Required Sample:</label>
                    <textarea name="sample" value={this.state.sample} onChange={this.onInputChange} className="form-control col-md-4" ></textarea>
                </FormGroup>
                <Notifications />
                <button type="submit" className="btn btn-outline-primary offset-md-5" title="add to list"><FaPlus />Add Request</button>                 
                <LabRequestTable 
                ...
                <button className="btn btn-outline-success" onClick={this.handleSubmit}>
                    <Link to="/patient_clarking/prescription_request" style={{width:'9vw'}}><FaArrowLeft /> Prev</Link>
                </button>
...
export const LabRequestTable = ({ formRecords=[], showRemove=true, onRemove=f=>f }) => {
    const rowStyle = {
    ...
    formRecords.forEach((record, i) => {
        rows.push(
            <tr key={i}>
                ...
                { showRemove && <td style={{cursor:'pointer'}}><Badge color="danger" pill onClick={() => onRemove(record)}>Remove</Badge></td>}
            </tr>
        )
    })
    return (
            ...
            <Table hover bordered striped cellspacing={0} cellpadding={0}>
                <thead>
                    <tr>
                        ...
                        { showRemove && <th style={rowStyle}></th>}
                    </tr>
                </thead>
                <tbody>
                    {rows}
