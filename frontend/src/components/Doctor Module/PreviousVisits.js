...
class Row extends React.Component {
    ...
    render(){
        ...
        return(
            <tr style={{ cursor:'pointer' }} key={diagnosis.transactionId} onClick={this.handleFetchDetails}>
                <td>{diagnosis.transactionId}</td>
                <td>{diagnosis.date}</td>
                <td><a style={{ marginBottom: '1rem' }}>{diagnosis.seen_by}</a> </td>
                <td>{diagnosis.id}</td>
            </tr>
        )
    }
        ...
        <div style={{width: '99%', height: '400px'}}>
            <FreeScrollBar>
                <div>
                    {
                        newDiag.length === 0 ? 
                        <p style={{textAlign:'center', paddingTop: '2em'}}><em>This patient does not have any record with this hospital</em></p>
                        :
                        <table className="table table-hover col-md-2 ">
                            ...
                            <tbody>
                                {
                                    newDiag.map(diagnosis => (
                                        <Row 
                                            key={diagnosis.transactionId} 
                                            diagnosis={diagnosis}
                                            toggleCollapse2={this.props.toggleCollapse2} 
                                            getFullDiagnosisByTransactionId={this.props.getFullDiagnosisByTransactionId}
                                        />
                                    ))
                                }
                            </tbody>
